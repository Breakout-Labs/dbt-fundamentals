...
survey_response as (
    select * from {{ 'stg_sheets__customer_survey_responses' }}
),
...
joined as (
    ...
    left join survey_response on (
        customers.email = survey_response.customer_email
    )
)
