...
customer_metrics as (
    select
        ...
        {% for days in [30, 90, 360] %}
            count_if(datediff('day',ordered_at,current_date())<={{days}}) as count_orders_last_{{days}}_days
            {% if not loop.last %}
                ,
            {% endif %}
        {% endfor %}
...
joined as (
    ...
    left join stg_survey_response on (
        customers.email = stg_survey_response.customer_email
    )
)
